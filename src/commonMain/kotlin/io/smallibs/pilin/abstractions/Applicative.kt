...
object Applicative {
    ...
    interface WithPureAndApply<F> : Core<F> {
        override suspend fun <A, B> map(f: Fun<A, B>): Fun<App<F, A>, App<F, B>> = apply(pure(f))
    ...
    open class Operation<F>(private val c: Core<F>) : Core<F> by c {
        suspend fun <A, B> lift1(f: Fun<A, B>): Fun<App<F, A>, App<F, B>> = map(f)
        ...
        suspend fun <A, B> discardRight(ma: App<F, A>): Fun<App<F, B>, App<F, A>> = lift2<A, B, A>(Standard::const)(ma)
    }
    ...
    open class Infix<F>(private val c: Core<F>) : Functor.Infix<F>(c), Core<F> by c {
        ...
        suspend infix operator fun <A, B> App<F, A>.times(mb: App<F, B>): App<F, Pair<A, B>> = c.product<A, B>(this)(mb)

        suspend infix fun <A, B> App<F, A>.product(mb: App<F, B>): App<F, Pair<A, B>> = this * mb

        suspend infix fun <A, B> App<F, Fun<A, B>>.apply(ma: App<F, A>): App<F, B> = c.apply(this)(ma)

        suspend infix fun <A, B> App<F, A>.discardLeft(mb: App<F, B>): App<F, B> = o.discardLeft<A, B>(this)(mb)

        suspend infix fun <A, B> App<F, A>.discardRight(mb: App<F, B>): App<F, A> = o.discardRight<A, B>(this)(mb)
