...
object Applicative {
    private class ApplicativeImpl<F, E>(val inner: Applicative.Core<F>) : Applicative.API<ReaderK<F, E>>,
        Applicative.WithPureAndApply<ReaderK<F, E>> {
        override suspend fun <A> pure(a: A): App<ReaderK<F, E>, A> = Reader(Standard.const(inner.pure(a)))
    ...
    fun <F, E> applicative(inner: Applicative.Core<F>): Applicative.API<ReaderK<F, E>> = ApplicativeImpl(inner)
}